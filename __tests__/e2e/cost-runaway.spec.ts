...
test.describe('Cost Runaway Prevention', () => {
  test('should detect threshold breach and send alert', async ({ page, request }) => {
    ...
    await page.click('button:has-text("프로젝트 생성"), button:has-text("Create Project")');
    ...
    if (await thresholdInput.count() > 0) {
      ...
      await page.click('button:has-text("임계값 설정"), button:has-text("Set Threshold")');

      // 설정 성공 확인
      await expect(page.locator('text=/임계값.*설정|Threshold.*set/i')).toBeVisible({ timeout: 10000 });
    }
    ...
    // 비활성화 버튼 클릭
    const disableButton = page.locator('button:has-text("비활성화"), button:has-text("Disable")');
    if (await disableButton.count() > 0) {
      ...
      if (await confirmInput.count() > 0) {
        ...
        await expect(page.locator('text=/비활성화됨|Disabled/i')).toBeVisible({ timeout: 10000 });
      }
    }
  ...
  test('should prevent API calls with disabled key', async ({ page, request }) => {
    ...
    if (await disabledProject.count() > 0) {
      ...
      const warningMessage = page.locator('text=/이 API 키는 비활성화|This API key is disabled/i');
