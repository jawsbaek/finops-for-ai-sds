...
test.describe("Weekly Report", () => {
	...
	test("should display weekly report with top and bottom projects", async ({
		page,
	}) => {
		// 리포트 페이지로 이동
		...
		if ((await top3Section.count()) > 0) {
			...
			const projectItems = page.locator(
				'[data-testid="top-project"], .top-project-item',
			);
		...
		const bottom3Section = page.locator(
			"text=/Bottom 3|개선 필요|improvement/i",
		);
		if ((await bottom3Section.count()) > 0) {
			await expect(bottom3Section.first()).toBeVisible();

			const bottomProjectItems = page.locator(
				'[data-testid="bottom-project"], .bottom-project-item',
			);
	...
	test("should show weekly metrics and trends", async ({ page }) => {
		...
		// 주간 총 비용 확인
		const totalCost = page.locator(
			"text=/주간 총 비용|Total Weekly Cost|\\$\\d+/i",
		);
		...
		// 전주 대비 증감률 확인
		const weekOverWeek = page.locator(
			"text=/전주 대비|week-over-week|WoW|\\+\\d+%|-\\d+%/i",
		);
	...
	test("should access report archive and view historical reports", async ({
		page,
	}) => {
		await page.goto("/reports");
		...
		const archiveLink = page.locator(
			'a:has-text("아카이브"), a:has-text("Archive"), button:has-text("Past Reports")',
		);
		if ((await archiveLink.count()) > 0) {
			...
			// 아카이브 페이지 확인
			await expect(page.locator("h1, h2")).toContainText(
				/아카이브|Archive|과거 리포트/i,
			);

			// 과거 리포트 목록 확인
			const reportList = page.locator(
				'[data-testid="report-item"], .report-list-item',
			);
			if ((await reportList.count()) > 0) {
				...
				// 리포트 상세 내용 표시 확인
				await expect(
					page.locator("text=/Top 3|Bottom 3|주간|Weekly/i"),
				).toBeVisible();
			}
		}
	});

	test("should trigger weekly report generation manually", async ({
		page,
		request,
	}) => {
	...
	test("should display project efficiency scores", async ({ page }) => {
		...
		const efficiencyScores = page.locator(
			'[data-testid="efficiency-score"], .efficiency-value',
		);
	...
	test("should show cost breakdown by project", async ({ page }) => {
		...
		const costBreakdown = page.locator(
			"text=/프로젝트별 비용|Cost by Project/i",
		);
	...
	test("should allow filtering reports by date range", async ({ page }) => {
		...
		if ((await dateFilter.count()) > 0) {
			...
			const applyButton = page.locator(
				'button:has-text("적용"), button:has-text("Apply")',
			);
