...
test.describe('Epic 1 - 비용 급증 감지 및 즉시 대응', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.route('https://api.openai.com/**', (route) => {
      route.fulfill({
        ...
        body: JSON.stringify({
          data: [
            { timestamp: Date.now(), aggregation_timestamp: Date.now() / 1000, n_requests: 10, operation: 'chat.completions', n_context_tokens_total: 1000, n_generated_tokens_total: 500 }
          ]
        }),
      });
    });
  ...
  test('complete user journey from signup to cost runaway prevention', async ({ page }) => {
    ...
    await page.click('button:has-text("팀 생성"), button:has-text("Create Team")');
    ...
    await page.click('button[type="submit"], button:has-text("생성"), button:has-text("Create")');
    ...
    await page.click('button:has-text("프로젝트 생성"), button:has-text("Create Project"), button:has-text("New Project")');

    await page.waitForSelector('input[name="name"], input[name="projectName"]');
    await page.fill('input[name="name"], input[name="projectName"]', projectName);
    ...
    // API 키 등록 버튼 찾기
    await page.click('button:has-text("API 키 등록"), button:has-text("Register API Key"), button:has-text("Add API Key")');

    await page.waitForSelector('input[name="apiKey"]');
    await page.fill('input[name="apiKey"]', 'sk-test-mock-key-12345678901234567890123456789012');
    await page.click('button[type="submit"], button:has-text("저장"), button:has-text("Save")');

    // API 키 등록 성공 확인
    await expect(page.locator('text=/API 키.*등록|API Key.*registered/i')).toBeVisible({ timeout: 10000 });
    ...
    // 임계값 설정 섹션 찾기
    const thresholdInput = page.locator('input[name="dailyThreshold"], input[name="threshold"]');
    if (await thresholdInput.count() > 0) {
      await thresholdInput.first().fill('500');
      await page.click('button:has-text("임계값 설정"), button:has-text("Set Threshold"), button:has-text("Save")');

      // 임계값 설정 성공 확인
      await expect(page.locator('text=/임계값|threshold/i')).toBeVisible({ timeout: 10000 });
    }
    ...
    const disableButton = page.locator('button:has-text("비활성화"), button:has-text("Disable"), button:has-text("Deactivate")');
    if (await disableButton.count() > 0) {
      await disableButton.first().click();

      // Type-to-confirm 모달 확인
      const confirmInput = page.locator('input[placeholder*="차단"], input[placeholder*="disable"]');
      if (await confirmInput.count() > 0) {
        await confirmInput.first().fill('차단');
        await page.click('button:has-text("확인"), button:has-text("Confirm")');

        // 비활성화 성공 확인
        await expect(page.locator('text=/비활성화됨|비활성화 완료|Disabled|Deactivated/i')).toBeVisible({ timeout: 10000 });
      }
    }
  ...
  test('weekly report access', async ({ page }) => {
    ...
    const reportContent = page.locator('text=/Top 3|Bottom 3|효율|efficiency/i');
